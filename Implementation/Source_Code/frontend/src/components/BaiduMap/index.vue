      ...
      <bm-marker 
       v-for="(item,index) in pointArr"
        ...
        <bm-info-window :show="item.st" @close="infoWindowClose" style="width:250px" class="infoBox" @open="infoWindowOpen">
           <p><span class="tit">equipment number: </span> <span>{{item.deviceId}}</span></p>
           <p><span class="tit">equipment status: </span><span>{{item.online ? 'online' : 'offline'}}</span></p>
           <p class="list"><span class="tit">alarm index: </span>
