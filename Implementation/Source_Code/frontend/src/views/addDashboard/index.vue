  ...
  <div class="addDishBoard-container">
    <div class="topHander">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>Add panel</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="content">
      <el-form :model="form" label-width="100px" style="padding:10px; position: relative;" ref="form" label-position="left" >
        <el-form-item label="panel name：" prop="name" >
          <el-input v-model="form.name" size="small" placeholder="Please enter the name of the board" ></el-input>
        </el-form-item>
        <div class="fromRow">
          <el-form-item label="select indicator" prop="quota"  >
            <el-select
              v-model="form.quota"
              filterable
              placeholder="Indicators (single choice)"
              size="small"
              class="select-box"
          ...
          <div class="fromCol">
            <el-select v-model="form.deviceIdList" filterable multiple placeholder="Equipment (multiple choice)" size="small" :multiple-limit=5 class="select-box" >
              <el-option v-for="item in quotaDevicesArr" :key="item" :label="item" :value="item" />
            </el-select>
          </div>
          
        </div>  
        <div class="butLook" @click="lookHandle"> preview </div> 
      </el-form>
      ...
      <div class="submit">
        ...
        <el-button type="primary" size="small" @click="onSubmit" style="margin-left:30px" >confirm</el-button>
      </div>
    </div>
  </div>
</template>
...
export default class extends Vue {
  ...
  async lookHandle(type?:number){
    ...
    await preview(params).then((res: any) => {
        if (res.xdata) {
          ...
          this.$message.error('The selected device has no data to preview for now!')
        }
    })
  ...
  async onSubmit() {
    ...
    await addBoard(params).then((res: any) => {
        if (res) {
          this.$message.success('The board added successfully!')
          this.cancel()
        }
    })
  }
  ...
  verification(){
    ...
    if (!verData.name || verData.name.trim() == '') {
      this.$message.error('Please enter the board name')
      return false
    }
    if (!verData.quota || String(verData.quota).trim() == '') {
      this.$message.error('Please choose an indicator')
      return false
    }
    if (verData.deviceIdList.length == 0) {
      this.$message.error('Please choose an equipement')
      return false
    }
    return true
  }
