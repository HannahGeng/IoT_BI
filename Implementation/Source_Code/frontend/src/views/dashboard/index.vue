<template>
  <div class="dashboard-container" @click.stop="() =>  boardShow ? boardShow = false : null">
    <div class="mapCont">
      <div class="title">Equipment distribution</div>
      <!-- <div class="map">
    ...
    <div class="content">
      <div :class="{listCont:true, wtA:true , bigWit: !boardSystemBoard.some(it => it.id == 3)}" v-if="boardSystemBoard.some(it => it.id == 2)">
        <div>
          ...
          <div class="title"><span>Abnormal equipment(today)</span><span class="mon" @click="() => $router.push('/alarmLog/list')">more></span></div>
          <div class="table">
            <div class="head">
              <span  style="width:200px; flex:none">alarm time</span>
              <span>alarm name</span>
              <span  style="width:120px; flex:none">level</span>
              <span>alarm quipment</span>
              <span style="width:80px; flex:none">data</span>
            </div>
            <div class="conttent" style="height: 200px;overflow: scroll;">
              <div class="trLine" v-for="it in tableData" :key="it.name">
                  ...
                  <span v-if="it.level == 2" style="width:120px; flex:none"><i>severe</i></span>
                  <span v-if="it.level == 1" style="width:120px; flex:none"><i class="pub">general</i></span>
                  <span>{{it.deviceId}}</span>
                  <span style="width:80px; flex:none">{{it.online ? `${it.value}${it.unit}` : 'offline'}}</span>  
              </div>
              <div v-if="tableData.length == 0" style="line-height: 200px;text-align: center;color: #999;">No abnormal equipment alarm at the moment!</div>
            </div>
          </div>
        </div>
      </div>
      <div :class="{listCont:true, wtB:true , bigWit: !boardSystemBoard.some(it => it.id == 2)}" v-if="boardSystemBoard.some(it => it.id == 3)">
        ...
        <div class="title">
            <span>Abnormal trend (unit: pcs)</span>
      ...
      <div :class="{listCont:true, bigWit: unSystemBoardList.length == 0}" v-if="boardSystemBoard.some(it => it.id == 4)">
        ...
        <div class="title">
          <span>Abnormal count TOP10(unit:pcs)</span>
    ...
    <div class="addDashboard">
      <div class="addBut" @click.stop="() => boardShow = true">edit dashboard
      <div class="dishboardSelect" v-show="boardShow">
        <span @click="openAddBoard">edit dashboard</span>
        <span @click="() => $router.push('/addDashboard')">+add dashboard</span>
      </div>
      </div>
    </div>
    <el-dialog
    ...
    >
      ...
      >edit dashboard</div>
        <el-checkbox-group v-model="checkList" class="checkbox">
          <el-checkbox label="1" disabled>equipment distribution</el-checkbox>
          <el-checkbox label="2">abnormal trend</el-checkbox>
          <el-checkbox label="3">abormal trend</el-checkbox>
          <el-checkbox label="4">abnormal TOP10</el-checkbox>
        </el-checkbox-group>
      <div
        slot="footer"
        class="dialog-footer"
...
export default class extends Vue {
  ...
  // 异常设备实时数据 今天
  getMqttData(){
    var client  = mqtt.connect('ws://192.168.2.8/mqtt', {
      port: 8083,
